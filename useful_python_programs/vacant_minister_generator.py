...
def get_tags():
	tag_files = open("../common/country_tags/00_countries.txt", "r+")
	tags = []
...
def make_blank_chars(tags):
	output_file = open("../common/characters/_blank_characters.txt", "w+")
	output_file.write("characters = {\n")

	for tag in tags:
		if not check_folder(f'{tag}_Generic_Vacant_hog = {{', Path("..", "common", "characters")):
			text = f"""
		{tag}_Generic_Vacant = {{
			name = Generic_Vacant_Minister
			portraits = {{
...
def add_slots_as_traits():
	characters_folder = Path("..", "common", "characters")
